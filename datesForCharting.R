...
asDDMM <- function(dateOrg = '2013-04-02'){
  ...
  return(paste(paste(substr(dateOrg, 6,7), "/",substr(dateOrg, 9,10),sep ="")))
...
seqOFDates<-function(from = from, to = to, datediff_ = 'day') {
  ...
  switch (datediff_,
    'day' = {
        ...
        datesAll  <- lapply(datesAll, asDDMM)
        
        #strip year part -  redundant and repeating
        return(unlist(datesAll))
    ...
    'month' = {
      return(
        # Monthly
        #seq.Date(from = as.Date(from), to = as.Date(to), by = 'month')
        as.yearmon(seq.Date(from = as.Date(from), to = as.Date(to), by = 'month'))
    ...
    'year' = {
      ...
      return(
        # yearly
        #seq.Date(from = as.Date(from), to = as.Date(to), by = 'year')
        as.yearmon(seq.Date(from = as.Date(from), to = as.Date(to), by = 'year'))
    ...
    'week' = {
      ...
      return(# weekly
        #seq.Date(from = as.Date(from), to = as.Date(to), by = 'week')
        as.yearmon(seq.Date(from = as.Date(from), to = as.Date(to), by = 'week'))
      )
    }
